    ...
    return (
        <div className='w-full h-[80px] flex items-center justify-between border-b-2 border-gray-800'>
            <div className='flex items-center gap-[10px]'>
                ...
                <div className='w-[60px] h-[60px] border-2 border-black rounded-full cursor-pointer overflow-hidden' onClick={() => navigate(`/profile/${user.userName}`)}>
                    <img src={user.profileImage || dp} alt="" className='w-full object-cover' />
                </div>
                <div>
                    <div className='text-[18px] text-white font-semibold'>{user.userName}</div>
                    <div className='text-[18px] text-gray-400 font-semibold'>{user.name}</div>
                </div>
            </div>
